function init() {
  ...
  function decrement() {
    ...
    if (timer === 0) {
      ...
      if (score > 1500) {
        timeKeeper.innerHTML = 'parabéns, Você ganhou! 🎉'
        if (score > highScorePressure) {
          localStorage.setItem('highScorePressure', score)
        }
      } else {
        timeKeeper.innerHTML = 'Você ficou sem tempo 🥵'
      }
      clearGrid()
  ...
  function checkMoves() {
    if (mode === 'pressure' && !firstMove) {
      ...
      if (moves === 0) {
        grid.style.display = 'none'
        if (score > 500) {
          movesKeeper.innerHTML = 'parabéns, Você ganhou! 🎉'
          if (score > highScoreStrategy) {
            localStorage.setItem('highScoreStrategy', score)
          }
        } else {
          movesKeeper.innerHTML = 'Você ficou sem tempo 🥵'
  ...
  function createBoard() {
    ...
    scoreKeeper.innerHTML = `Score: ${score}`
    movesKeeper.innerHTML = 'Você tem 10 movimentos para ganhar 500 pontos'
    timeKeeper.innerHTML = 'Você tem 1 minuto para ganhar 1.500 pontos'
    highScorePressure = localStorage.getItem('highScorePressure')
    highScoreStrategy = localStorage.getItem('highScoreStrategy')
    if (mode === 'pressure') {
      highScoreKeeper.innerHTML = `👑 pontuação máxima: ${highScorePressure}`
    } else if (mode === 'strategy') {
      highScoreKeeper.innerHTML = `👑 pontuação máxima: ${highScoreStrategy}`
    }
